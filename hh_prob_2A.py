...
def htmC(c,b_grid): # example of adding a het output, gets saved as steady state and agg is returned  
...
def rbgrid(a_grid,b_grid,z_grid,rb,borwedge):
    rb_grid=rb*np.ones(b_grid.shape[0])
    borFlag=sum(b_grid<0)
    rb_grid[0:borFlag]=rb_grid[0:borFlag]+borwedge
    return rb_grid


# policy and bacward order as in grid!
@het(exogenous='Pi', policy=['b', 'a'], backward=['Vb', 'Va'],
     hetinputs=[marginal_cost_grid,rbgrid], hetoutputs=[adjustment_costs,htmC], backward_init=hh_init)  
def hh(Va_p, Vb_p, a_grid, b_grid, z_grid, e_grid, k_grid, beta, eis, rb, ra, chi0, chi1, chi2, Psi1,rb_grid,borwedge,cbar):
    ...
    c_endo_unc = interpolate.apply_coord(i, pi, Wb) ** (-eis) + cbar
    ...
    b_endo[b_endo<0]=b_endo[b_endo<0]*(1+rb)/(1+rb+borwedge)
    ...
    c_endo_con = ((1 + k_grid[np.newaxis, :, np.newaxis]) ** (-eis)
                  * interpolate.apply_coord(i, pi, Wb[:, 0:1, :]) ** (-eis)) + cbar
    ...
    c = addouter(z_grid, (1 + rb_grid) * b_grid, (1 + ra) * a_grid) - Psi - a - b
    uc = (c-cbar) ** (-1 / eis)*(c>cbar)+(c<=cbar)*10e6
    ...
    #Va = 0.99*Va+0.01*Va[1,20,0]
    #Vb = 0.99*Vb +0.01*Vb[1,20,0]
