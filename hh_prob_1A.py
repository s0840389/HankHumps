...
def hh(Va_p, a_grid, we, T, r, beta, eis, frisch, vphi,cbar):
    '''Single backward step via EGM.'''
    uc_nextgrid = beta * Va_p
    c_nextgrid, n_nextgrid = cn(uc_nextgrid, we[:, np.newaxis], eis, frisch, vphi,cbar)
    ...
    if iconst[0].size != 0 and iconst[1].size != 0:
        c[iconst], n[iconst] = solve_cn(we[iconst[0]],
                                        ...
                                        eis, frisch, vphi, Va_p[iconst],cbar)
...
@njit
def cn(uc, w, eis, frisch, vphi,cbar):
    """Return optimal c, n as function of u'(c) given parameters"""
    return uc ** (-eis)+cbar, (w * uc / vphi) ** frisch


def solve_cn(w, T, eis, frisch, vphi, uc_seed,cbar):
    uc = solve_uc(w, T, eis, frisch, vphi, uc_seed,cbar)
    return cn(uc, w, eis, frisch, vphi,cbar)


@vectorize
def solve_uc(w, T, eis, frisch, vphi, uc_seed,cbar):
    ...
    for i in range(30):
        ne, ne_p = netexp(log_uc, w, T, eis, frisch, vphi,cbar)
        if abs(ne) < 1E-11:
            break
        else:
            log_uc -= ne / ne_p
...
@njit
def netexp(log_uc, w, T, eis, frisch, vphi,cbar):
    """Return net expenditure as a function of log uc and its derivative."""
    c, n = cn(np.exp(log_uc), w, eis, frisch, vphi,cbar)
    ne = c - w * n - T
